...
export default function() {
    ...
    try {
        ...
        if (!selectedTrigger) {
            throw new Error("Please select a trigger first");
        }
        const response = await axios.post(`${BACKEND_URL}/api/v1/zap`, {
            availableTriggerId: selectedTrigger.id,
            triggerMetadata: {},
            actions: selectedActions.map(a => ({
                availableActionsId: a.availableActionId, // Fixed property name
                actionMetadata: {}
            }))
        }, {
            headers: {
        ...
        if (axios.isAxiosError(err)) {
            setError(err.response?.data?.message || "Failed to create zap");
        }
    } finally {
        setIsLoading(false);
    }
